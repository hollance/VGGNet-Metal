...
/*
  ...
  Note that we flip RGB to BGR (inColor.x and inColor.z get swapped), since the
  tool that was used to train the network, Caffe, uses images with BGR pixels.
  
  The alpha component of outColor is not important, since our MPSImages only
  use the first 3 feature channels.
*/
kernel void adjust_mean(texture2d<float, access::read> inTexture [[texture(0)]],
